...
//             sharedMonthly: product.priceShared,   // from DB
//             sharedYearly: product.pricePrivate,// or product.priceSharedYearly if backend provides
//         };
...
//             const updatedProduct = await api.get(`/products/slug/${slug}`);
...
const ProductDetail = () => {
    const { slug } = useParams();
    const navigate = useNavigate(); // ✅ for redirection later if needed
    const [product, setProduct] = useState(null);
    const [reviews, setReviews] = useState([]);
    const [loading, setLoading] = useState(true);
    ...
    const handleBuyNow = () => {
        ...
        try {
            // Example action (replace with your payment or checkout route later)
            const orderDetails = {
                productId: product._id,
            ...
            // Example: Navigate to a checkout page with state
